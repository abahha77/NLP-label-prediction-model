...
id_coulmn=test_data['ID']

with open('D:/Deep Project/Models/Saved_Preprocessing_models/Transformer/tokenizer.pkl', 'rb') as f:
    tokenizer = pickle.load(f)

test_text = test_text.apply(lambda x: tokenizer.tokenize(str(x)))

word2vec_model = Word2Vec.load('D:/Deep Project/Models/Saved_Preprocessing_models/Transformer/word2vec_model.model')

test_embeddings = test_text.apply(lambda x: [word2vec_model.wv[word] for word in x if word in word2vec_model.wv])

with open('D:/Deep Project/Models/Saved_Preprocessing_models/Transformer/max_sequence_length.pkl', 'rb') as f:
...
model = load_model('D:/Deep Project/Models/Saved_Preprocessing_models/Transformer/model.h5')
...
result.to_csv('D:/Deep Project/Result_csv_files/Transformer_result.csv', index=False)